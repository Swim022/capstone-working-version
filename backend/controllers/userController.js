...
export const getUser = async (req, res) => {
 ...
 res.status(201).json({
    ...
    user,
        ...
        console.log(error);
        res.status(500).json({ status: "failed", messages: error.message });
    }
};
...
const {currentPassword, newPassword, confirmPassword} = req.body
 ...
 const isMatch = await omparePassword( currentPassword, user?.password);
...
/*export const signinUser = async(req, res)=> {
    try {
        ...
        console.log(error);
        req.status(500).json({ status: "failed", message: error.message });
    }
}*/